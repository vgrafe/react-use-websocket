...
export const useVinceSocket = <T = unknown>(
  url: string | (() => string | Promise<string>) | null,
  options: Options = DEFAULT_OPTIONS,
  connect: boolean = true,
  throttleMs: number = 0
): VinceSocketHook<T> => {
  const messagesReceived = useRef<WebSocketEventMap["message"][]>([]);
    ...
    if (url !== null && connect === true) {
      ...
      const start = async () => {
        ...
        const protectedSetLastMessage = (
          ...
          if (!expectClose) {
            messagesReceived.current.push(message);
          }
        };
  ...
  useInterval(() => {
    if (messagesReceived.current.length === 0) return;

    if (lastMessages.length === messagesReceived.current.length) return;

    setLastMessages(messagesReceived.current);
    messagesReceived.current = [];
  }, throttleMs);
